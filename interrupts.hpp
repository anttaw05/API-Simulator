...
std::tuple<std::string, int, std::string> parse_trace(std::string trace) {
    ...
    if (parts.size() < 1) {
        std::cerr << "Malformed line: " << trace << std::endl;
        return {"null", -1, "null"};
    }

    auto activity = parts[0];
    std::string extern_file = "null";
    int duration_intr = -1;   // default

    // Trim leading spaces (important!)
    //activity.erase(0, activity.find_first_not_of(" \t"));

    if(activity == "EXEC") {
        extern_file = parts[1];
        //extern_file.erase(0, extern_file.find_first_not_of(" \t"));
    }
    else if(parts.size() > 1) {
        duration_intr = std::stoi(parts[1]);   // only convert when it's numeric
    }
